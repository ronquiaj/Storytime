...
function ArchivedStories(props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    archiveData.forEach((story) => {
      ...
      archive.push(<MiniStory title={title} text={text} emoji={emoji} classes={classes} dateCreated={dateCreated}/>)
    });
  ...
  return <Container className={classes.container}>
    <Row>{loading ? <Spinner /> : archivedPosts.length === 0 ? <h1 className={classes.noArchivedPosts}>There are no archived posts yet!</h1> : archivedPosts}</Row>
  </Container>;
}
